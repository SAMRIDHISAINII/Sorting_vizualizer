<template>
  <div id="app">
    ...
    <!-- nav bar -->
    <nav class="navbar navbar-expand-lg navbar-light navbar-light" style="background-color: #FFB6C1;">
...
export default {
  ...
  data: function () {
    return {
      ...
      arr: [...Array(120)].map(() => [Math.ceil(Math.random() * 99), 'purple']),
  ...
  watch: {
    arrSize: function (val) {
      this.arr = [...Array(parseInt(val))].map(() => [Math.ceil(Math.random() * 99), 'purple']);
  ...
  methods: {
    ...
    updateArr() {
      this.arr = [...Array(parseInt(this.arrSize))].map(() => [Math.ceil(Math.random() * 99), 'purple']);
      this.isInSortingProcess = false;
      this.sorted = false;
    },
    colorArr(color) {
